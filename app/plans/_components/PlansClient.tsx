...
const categoryInfo: Record<PlanCategory, { desc: string; color: string; tag: string }> = {
  Premium: {
...
export default function PlansClient() {
  const [activeCategory, setActiveCategory] = useState<PlanCategory>('Premium');
  const [selectedDuration, setSelectedDuration] = useState<'3m' | '6m' | '12m'>('3m');

  const filtered = plans.filter(p => p.category === activeCategory);
  const durationLabels = { '3m': '3 Months', '6m': '6 Months', '12m': '12 Months' };
  ...
  return (
    <div className="pt-16 bg-slate-950 min-h-screen">
      ...
      <div className="bg-gradient-to-br from-blue-900 to-blue-700 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-2 bg-slate-800/50 rounded-full px-3 py-1 mb-4">
            ...
            <span className="text-xs text-slate-300">Transparent Pricing, No Hidden Charges</span>
          </div>
          <h1 className="heading-rhythm text-4xl font-bold mb-3">Broadband Plans</h1>
          <p className="copy-rhythm text-slate-400 max-w-lg mx-auto">Choose from Premium, Budget, or Eco plans with speeds up to 100 Mbps. All plans include unlimited data.</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="bg-slate-900 rounded-2xl p-1.5 shadow-sm border border-slate-700 flex gap-1">
            {(['Premium', 'Budget', 'Eco'] as PlanCategory[]).map(cat => (
              <button
                ...
                className={`px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${activeCategory === cat
                  ? cat === 'Premium' ? 'bg-gradient-to-r from-blue-950 to-blue-600 text-white shadow-sm'
                    : cat === 'Budget' ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-sm'
        ...
        {/* Category Info */}
        <div className={`${categoryInfo[activeCategory].color} rounded-2xl p-5 mb-8 text-white flex flex-col sm:flex-row sm:items-center gap-3`}>
          <div className="flex-1">
            <span className="inline-block bg-slate-800/70 text-white text-xs font-semibold px-2.5 py-1 rounded-full mb-2">{categoryInfo[activeCategory].tag}</span>
            <p className="text-sm leading-relaxed opacity-90">{categoryInfo[activeCategory].desc}</p>
          </div>
          {activeCategory === 'Premium' && (
            <div className="flex items-center gap-2 bg-slate-800/60 rounded-xl px-4 py-2 shrink-0">
              <Star size={14} className="text-yellow-300 fill-yellow-300" />
              <span className="text-sm font-semibold">Festival Offers Available</span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <h2 className="text-xl font-bold text-slate-100">{activeCategory} Plans</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-14">
          {filtered.map(plan => {
            ...
            return (
              ...
              >
                {plan.badge && (
                  <div className={`absolute top-0 right-0 ${isPopular ? 'bg-blue-500' : 'bg-blue-600'} text-white text-xs font-bold px-3 py-1 rounded-bl-xl`}>
                    {plan.badge}
                  </div>
                )}
                <div className="p-5">
                  <div className="mb-4">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-sm text-slate-400 font-medium">Mbps</span>
                    </div>
                    <p className="text-xs text-slate-500">Fiber Broadband</p>
                  </div>
                  <div className="mb-4">
                    <p className="text-3xl font-bold text-slate-100">₹{variant.price.toLocaleString()}</p>
                    <p className="text-xs text-slate-400">for {durationLabels[selectedDuration]}</p>
                    {selectedDuration !== '3m' && (
                      <p className="text-xs text-emerald-300 font-medium mt-0.5">
                        ₹{Math.round(variant.price / variant.months).toLocaleString()}/mo avg
                      </p>
                    )}
                  </div>
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="subheading-rhythm text-2xl font-bold text-slate-100">OTT + Broadband Add-ons</h2>
          </div>
          <p className="copy-rhythm text-slate-400 mb-2">Enhance your internet plan with premium OTT streaming bundles.</p>
          <div className="inline-flex items-center gap-2 bg-blue-900/40 border border-blue-800 rounded-xl px-4 py-2.5 mb-8">
            ...
            <p className="text-sm text-blue-300 font-medium">
              Special Offer: Add Basic OTT plan with any broadband at just <strong>₹299/year</strong>
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {ottPlans.map(plan => (
            <div key={plan.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
              <div className="flex items-center gap-2 mb-1">
                <Tv size={14} className="text-blue-500" />
                <h3 className="font-semibold text-slate-100 text-sm">{plan.name}</h3>
              </div>
              {plan.highlight && (
                <p className="text-xs text-blue-400 font-medium mb-3">{plan.highlight}</p>
              )}
              <div className="flex flex-wrap gap-1 mb-4">
                {plan.apps.map(app => (
                  <span key={app} className="text-xs bg-blue-900/30 text-blue-200 px-2 py-0.5 rounded-full">{app}</span>
                ))}
              </div>
