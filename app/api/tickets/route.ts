...
async function sendComplaintEmails(options: {
  ...
  const customerText = [
    ...
    "",
    "You can use the ticket ID above to track or follow up on this issue.",
    "",
    "Regards,",
    ...
    if (user) {
      ...
      const ticket = await prisma.tickets.create({
        data: {
          ...
          issue_type: issue_type as ticket_issue_type,
      ...
      sendComplaintEmails({
        ...
        console.error("api/tickets email error:", err);
      });

      return NextResponse.json({ ok: true, complaintId: ticket.id, trackingCode });
    } else {
      if (!name || !phone || !email || !address || !city || !state || !pin_code) {
        return NextResponse.json(
          { error: "Name, phone, email, address, city, state, and pin_code are required for guest tickets" },
          { status: 400 }
        );
      }
