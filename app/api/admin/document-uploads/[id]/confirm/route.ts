...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const uploadedByType = user.role === "ADMIN" ? "ADMIN" : "TECHNICIAN";
    ...
    return NextResponse.json({
      ...
      file: {
        ...
        uploadedByType: result.file.uploaded_by_type,
        createdAt: result.file.created_at.toISOString(),
      },
    });
  } catch (err) {
    console.error("api/admin/document-uploads/[id]/confirm POST error:", err);
    return NextResponse.json({ error: "Failed to confirm manual document upload" }, { status: 500 });
  }
}
