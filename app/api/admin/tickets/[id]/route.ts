...
const VALID_STATUSES: ticket_status[] = [
  "PENDING_APPROVAL",
  "OPEN",
  "IN_PROGRESS",
  "RESOLVED",
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const safeComplaint = {
      ...
      assigned_at: safeDate(ticket.assigned_at),
      last_reminder_sent_at: safeDate(ticket.last_reminder_sent_at),
      tracking_code: ticket.tracking_code,
    };

    return NextResponse.json({ ticket: safeComplaint });
  } catch (err) {
    console.error("api/admin/tickets/[id] GET error:", err);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (hasStatus && !VALID_STATUSES.includes(status as ticket_status)) {
  ...
  if (status === "RESOLVED" && user.role === "TECHNICIAN") {
    ...
    if (!approvedOtp) {
      return NextResponse.json(
        { error: "OTP verification required before resolving this ticket" },
        { status: 403 }
      );
    }
  }
  ...
  try {
    ...
    if (user.role === "TECHNICIAN") {
      if (hasStatus && status === "IN_PROGRESS") {
        if (existing.status !== "OPEN") {
          return NextResponse.json({ error: "Only open tickets can be started" }, { status: 400 });
        }

        if (existing.assigned_technician_id && existing.assigned_technician_id !== user.userId) {
          return NextResponse.json({ error: "This ticket is assigned to another technician" }, { status: 403 });
        }
      }

      if (hasStatus && status === "RESOLVED") {
        if (existing.assigned_technician_id && existing.assigned_technician_id !== user.userId) {
          return NextResponse.json({ error: "You can only resolve your assigned ticket" }, { status: 403 });
        }
      }
    }
    ...
    console.error("api/admin/tickets/[id] PATCH error:", err);
